...
namespace RCT3ArchiveDumper2 {
	...
	unsigned long current_preview = 0;
...
#pragma region Troep
	...
	private: System::Windows::Forms::CheckBox^  opt_ge_terrain;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->opt_ge_terrain = (gcnew System::Windows::Forms::CheckBox());
			...
			this->tb_filename->Text = L"C:\\Users\\Joey\\Documents\\RCT3\\Structures\\RCT3.fc";
			// 
			// btn_browse
			// 
			...
			this->stacknum->Value = System::Decimal(gcnew cli::array< System::Int32 >(4) {80, 0, 0, 0});
			// 
			// opt_pixeldata
			// 
			...
			// groupBox6
			// 
			this->groupBox6->Controls->Add(this->opt_ge_terrain);
			...
			// opt_ge_terrain
			// 
			this->opt_ge_terrain->AutoSize = true;
			this->opt_ge_terrain->Location = System::Drawing::Point(327, 88);
			this->opt_ge_terrain->Name = L"opt_ge_terrain";
			this->opt_ge_terrain->Size = System::Drawing::Size(80, 17);
			this->opt_ge_terrain->TabIndex = 29;
			this->opt_ge_terrain->Text = L"GE_Terrain";
			this->opt_ge_terrain->UseVisualStyleBackColor = true;
			// 
			// opt_vendor
			// 
			...
			this->Name = L"Form1";
			this->ShowIcon = false;
			this->Text = L"RCT3 - Public General Archive dumper - Version 2.0 build 22";
...
#pragma endregion
		...
		bool ReadDataType(FILE *&rar, FILE *&f, ARCVariableDef vars, char* childfrom, unsigned long level){
			...
			//uint8_t u8;
			char ref[8];
			unsigned long ulref[2];
			unsigned char uref[8];
			...
			if(strcmp("int32", vars.datatype) == 0){
				...
				/*if(strcmp(vars.name, "DatType") == 0 && (templong != 0 && templong != 1)){
					MessageBox::Show("DatFile doesn't seem to be of known type (type = " + templong + ")");
					return false;
				}*/
				...
				fread(&ulref, 4, 2, rar);
				//fread(&ref, 8, 1, rar);
				//sprintf_s(ltrtmp, 100, "%X%X%X%X%X%X%X%X", ref[0], ref[1], ref[2], ref[3], ref[4], ref[5], ref[6],ref[7]);
				sprintf_s(strtmp, 100, "<a href='#ptr%X-%X'>%X-%X</a>", ulref[0], ulref[1], ulref[0], ulref[1]);				
				...
				fgetpos(rar,&pos1);
				fread(&uref[0], 1, 1, rar);
				fread(&uref[1], 1, 1, rar);
				sprintf_s(sstr, 200, "X%d(west-east) Y%d(north-south)", uref[0], uref[1]);
				PostVar(f, pos1,level, ToChar(gcnew String(vars.name) + "(GE_Terrain)"), sstr);

				float tfloat = 0;
				fgetpos(rar,&pos1);
				fread(&tfloat, 4, 1, rar);
				PostVar(f, pos1,level, "GE_UNK1", tfloat);

				fgetpos(rar,&pos1);
				fread(&tfloat, 4, 1, rar);
				PostVar(f, pos1,level, "GE_UNK2", tfloat);

				fgetpos(rar,&pos1);
				fread(&tfloat, 4, 1, rar);
				PostVar(f, pos1,level, "GE_XTILESIZE", tfloat);

				fgetpos(rar,&pos1);
				fread(&tfloat, 4, 1, rar);
				PostVar(f, pos1,level, "GE_YTILESIZE", tfloat);

				unsigned int num_tiles = uref[0] * uref[1];
				if(opt_ge_terrain->Checked)
				{
					for(unsigned long i = 0; i < num_tiles; i++){
						fgetpos(rar,&pos1);
						fread(&tfloat, 4, 1, rar);
						fread(&tfloat, 4, 1, rar);
						fread(&tfloat, 4, 1, rar);
					...
					for(unsigned long i = 0; i < num_tiles; i++){
						fgetpos(rar,&pos1);
						fread(&tfloat, 4, 1, rar);
						PostVar(f, pos1,level, "CornerAHeight", tfloat);

						fgetpos(rar,&pos1);
						fread(&tfloat, 4, 1, rar);
						PostVar(f, pos1,level, "CornerBHeight", tfloat);

						fgetpos(rar,&pos1);
						fread(&tfloat, 4, 1, rar);
						PostVar(f, pos1,level, "CornerCHeight", tfloat);

						fgetpos(rar,&pos1);
						fread(&tfloat, 4, 1, rar);
						PostVar(f, pos1,level, "CornerDHeight", tfloat);

						fgetpos(rar,&pos1);
						fread(&tfloat, 4, 1, rar);
						PostVar(f, pos1,level, "Unk5", tfloat);

						fgetpos(rar,&pos1);
						fread(&tfloat, 4, 1, rar);
						PostVar(f, pos1,level, "Unk6", tfloat);
						WitRegel(f);
					}
				}

				// 9 unknowns, dan data (24bytes per blok)
			...
			if(strcmp("struct", vars.datatype) == 0){
				...
				if(strcmp(vars.name, "DatPreview") == 0){
					...
					tbmp->Save("dumps/" + Path::GetFileNameWithoutExtension(tb_filename->Text) + "/DatPreview" + current_preview + ".bmp");
					fseek(rar, origin, SEEK_SET);
					current_preview++;
				}
				if(num_array_items > 0 && size > 4)
				{
				 ...
				 if(ddata->Checked == true){
					 try{
						 for(unsigned long i = 0; i < arc.num_data_blocks; i++){
							 ...
							 char* sstr = (char*)malloc(100);
							 char* sstr2 = (char*)malloc(100);
							 char* check_str = (char*)malloc(100); // String voor checksum research
							 fprintf(f, "%s", returnHeader("Data block " + i));
							 fgetpos(rar,&pos1);
							 ...
							 sprintf_s(sstr, 100, "<a name='ptr%X-%X'>%X-%X</a>", arc.cdhd[i].unk1, arc.cdhd[i].unk2, arc.cdhd[i].unk1, arc.cdhd[i].unk2);
							 sprintf_s(check_str, 100, "0x%X 0x%X 		", arc.cdhd[i].unk1, arc.cdhd[i].unk2);
							 ...
							 Post(f, pos1, "ReferencePointer", sstr , "\n-------------------------------------------\n");
							 ...
							 for(unsigned long j = 0; j < arc.cdh[class_prototype].root_variable_count; j++){
								 if(ReadDataType(rar, f, arc.cdh[class_prototype].vars[j], "root", 0) == false){
									 ...
									 num_sid_str = 0;
									 if(debug_log){
										 unsigned long t = 3562;
										 fwrite(&t, 4, 1, debug_log);
										 fclose(debug_log);
									 }
									 return;
								 }
						 ...
						 char* tstr = (char*)malloc(200);
						 sprintf_s(tstr, 200, "An unknown error has been caught in the dump main function @block_parser: %s", e.what());
						 fprintf(f, "Fatal error: The dumper ended here duo to an error");
						 DebugLog(gcnew String(tstr));
					 } catch (...){
						 fprintf(f, "Fatal error: The dumper ended here duo to an error");
					 ...
					 if((pos2-pos1) == 4){
						 ...
						 fgetpos(rar,&pos1);
						 fread(&templong, 4, 1, rar);
						 Post(f, pos1, "Checksum", templong );
						 Post(f, pos1, "(HEX)", ToHex(templong));
